...
### Category Sort Order
Categories in the Category Manager modal can be reordered using the **▲ / ▼** arrow buttons on the right of each row. New order is saved immediately to `categories.json`.
...
Right-clicking any WO card opens a two-panel context menu. The **left rail** lists all actions (Tenant Email, Tenant Text, Vendor Email, Vendor Text, Follow-up, Category). Clicking a rail item slides in the right panel with the relevant options. The menu stays open while toggling categories. Changes are persisted immediately to `wo-tags.json`.

### Category Manager — Reorder
Categories in the Category Manager modal can be reordered using the **▲ / ▼** buttons on the right side of each row. Order is persisted immediately to `categories.json`.
