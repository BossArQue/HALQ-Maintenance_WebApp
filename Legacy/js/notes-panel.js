// =====================
// NOTES PANEL — Notebook/Section/Page Tree, Editor, Drawing, Export
// Registers: HALQ.notes
// =====================
(function () {
  'use strict'
  ...
  async function savePage () {
    ...
    await window.halq.notesPageSave(S.current.pageId, content)
    await _saveMeta()
    S.dirty = false
    renderTree()
    renderPgPanel()
  ...
  async function _saveMeta () {
    await window.halq.notesMetaSave(S.meta)
  }
  ...
  async function nImage () {
    ...
    if (!fr.ok) return
    const saved = await window.halq.notesAssetSave(S.current.pageId, fileName, fr.base64)
    if (!saved.ok) return
    $.body?.focus()
  ...
  async function ntPaste (e) {
    ...
    for (const item of items) {
      ...
      reader.onload = async (ev) => {
        ...
        const saved = await window.halq.notesAssetSave(S.current.pageId, `paste-${Date.now()}.png`, b64)
        if (saved.ok) {
  ...
  async function ntDrop (e) {
    ...
    for (const file of files) {
      ...
      reader.onload = async (ev) => {
        const b64   = ev.target.result.split(',')[1]
        const saved = await window.halq.notesAssetSave(S.current.pageId, file.name, b64)
        if (!saved.ok) return
  ...
  async function nSaveDrawing () {
    ...
    const saved  = await window.halq.notesAssetSave(S.current.pageId, `drawing-${Date.now()}.png`, b64)
    if (!saved.ok) return
    $.body?.focus()
  ...
  async function ntExportRun () {
    ...
    HALQ.showDebug('✓ Exported → ' + result.filePath.split(/[\\/]/).pop())
  }
